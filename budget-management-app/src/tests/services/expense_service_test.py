...
class TestExpenseService(unittest.TestCase):
    ...
    def test_add_expense_with_valid_data_works(self):
        ...
        expense_service.add_expense(self.expense.description, self.expense.amount, self.expense.date, self.valid_user.username)

        expenses = expense_repository.get_expenses_by_user(self.valid_user.username)
    ...
    def test_add_expense_fails_with_missing_fields(self):
        ...
        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense("", self.expense.amount, self.expense.date, self.valid_user.username)
        self.assertEqual(str(context.exception), "All fields are required!")

        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense(self.expense.description, "", self.expense.date, self.valid_user.username)
        self.assertEqual(str(context.exception), "All fields are required!")

        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense(self.expense.description, self.expense.amount, "", self.valid_user.username)
        self.assertEqual(str(context.exception), "All fields are required!")
    ...
    def test_add_expense_fails_with_non_positive_amount(self):
        ...
        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense(self.expense.description, 0, self.expense.date, self.valid_user.username)
        self.assertEqual(str(context.exception), "Amount must be greater than zero!")

        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense(self.expense.description, -100, self.expense.date, self.valid_user.username)
        self.assertEqual(str(context.exception), "Amount must be greater than zero!")
    ...
    def test_add_expense_fails_with_invalid_amount(self):
        ...
        with self.assertRaises(InvalidExpenseError) as context:
            expense_service.add_expense(self.expense.description, "invalid", self.expense.date, self.valid_user.username)
        self.assertEqual(str(context.exception), "Amount must be a valid number!")
    ...
    def test_update_expense_with_valid_data_works(self):
        ...
        expense_service.add_expense("car", 2000, "2025-04-08", self.valid_user.username)
        expenses = expense_repository.get_expenses_by_user(self.valid_user.username)
        self.assertEqual(len(expenses), 1)
        ...
        updated_expenses = expense_repository.get_expenses_by_user(self.valid_user.username)

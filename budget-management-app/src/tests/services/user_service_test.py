...
class TestUserService(unittest.TestCase):
    ...
    def test_create_user_fails_with_empty_username_or_password(self):
        ...
        self.assertEqual(str(context2.exception),
                         "Username and password must not be empty values!")
    ...
    def test_create_user_fails_with_invalid_username_or_password(self):
        with self.assertRaises(Exception) as context1:
            user_service.create_user(
                self.invalid_user.username, "1234", "1234")
        self.assertEqual(str(context1.exception),
                         "Password and username must have legth of at least 4!")

        with self.assertRaises(Exception) as context2:
            user_service.create_user(
                self.valid_user.username, self.invalid_user.password, self.invalid_user.password)
        self.assertEqual(str(context2.exception),
                         "Password and username must have legth of at least 4!")
    ...
    def test_login_fails_with_invalid_username_or_password(self):
        ...
        with self.assertRaises(Exception) as context2:
            user_service.login(self.valid_user.username,
                               self.invalid_user.password)
        self.assertEqual(str(context2.exception),
                         "Invalid username or password")

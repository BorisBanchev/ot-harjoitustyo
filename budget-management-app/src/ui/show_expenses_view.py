...
class ShowExpensesView:
    ...
    def _on_expense_select(self, event):
        ...
        if selected_expense:
            self._selected_expense_id = self._tree.item(selected_expense, "values")[0]
    ...
    def _refresh_budget(self):
        if user_service._user.monthly_budget is None:
            ...
            self._budget_label.config(text=f"Current Budget: {current_budget}€", foreground="green")
    ...
    def _refresh_expenses(self):
        ...
        for expense in expenses:
            self._tree.insert("", "end", values=(expense.id, expense.description, expense.amount, expense.date))

        self._refresh_budget()

    def _initialize_budget_field(self):
        self._budget_label = ttk.Label(master=self._frame, text="", foreground="green")
        self._budget_label.grid(row=0, column=0, padx=5, pady=5, sticky=constants.W)
    ...
    def _initialize_expenses_table(self):
        ...
        self._tree.grid(row=1, column=0, columnspan=3, padx=5, pady=5, sticky=constants.EW)
    ...
    def _initialize(self):
        ...
        delete_button.grid(row=2, column=0, padx=5, pady=5, sticky=constants.EW)
        ...
        add_expense_button.grid(row=2, column=2, padx=5, pady=5, sticky=constants.EW)
